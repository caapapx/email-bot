...
twinbox 当前架构：Loading/Thinking 分层脚本（Phase 1-4 均已 LLM 化）+ Python core 编排 + `scripts/twinbox_orchestrate.sh` 作为稳定 CLI 入口。分析 gastown 后发现阶段间/阶段内子任务可以并发，且 gastown 已经在工作目录里。

本方案定义如何将 twinbox 作为 rig 运行，使用 gastown 的 polecat/refinery/witness 实现多 agent 协作。

### 历史问题（已解决）

重构前 phase 脚本是纯 loading 管线——数据搬运 + regex 模式匹配，零 LLM 介入：
...
现已全部迁移到 loading → thinking 分层架构（详见下方迁移路径各阶段）。
...
- **并发子任务**（以 per-phase formula 为粒度，每个 phase 的 loading+thinking 作为一个 leg）：
  - Phase 1: `twinbox-phase1` formula（loading → intent 分类）
  - Phase 2: `twinbox-phase2` formula（loading → persona 推断）
  - Phase 3: `twinbox-phase3` formula（loading → lifecycle 建模）
