...
| **CLI** | `twinbox daemon start\|stop\|status\|restart`；**`start` 默认不带 `--supervise`**（可选：需要异常退出后自动再起时再加）；`cli_invoke` 支持 `cache_policy` / `timeout_ms`（见 [rpc-protocol.md](./rpc-protocol.md)） |
