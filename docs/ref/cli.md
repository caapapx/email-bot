...
twinbox onboard openclaw [--repo-root PATH] [--openclaw-home PATH] [--dry-run]
                         [--openclaw-bin NAME] [--skip-bridge] [--json]
```
...
twinbox deploy openclaw [--repo-root PATH] [--openclaw-home PATH] [--dry-run] [--no-restart]
                        ...
                        [--openclaw-bin NAME] [--skip-bridge] [--twinbox-bin PATH] [--json]
twinbox deploy openclaw --rollback [--remove-config] [--dry-run] [--no-restart] [--json]
```

**说明**：

- 成功部署后 JSON 报告含 `phase2_ready`、`plugin_tools`、`bridge` 等字段（与 `onboard openclaw --json` 同源）。
- `--skip-bridge`：高级逃生口，跳过 bridge 安装/健康检查（不推荐默认使用）。
- `--twinbox-bin`：写入 systemd unit 时使用的 **绝对路径** twinbox 可执行文件（默认自动解析）。
- `--rollback`：撤销上述接线（删除 `skills.entries.twinbox`、`~/.openclaw/skills/twinbox/`），并 **移除** vendor-safe bridge user units（`twinbox host bridge remove` 等价物）；**不删除** `~/.twinbox`；全量卸载见 `openclaw-skill/DEPLOY.md` §5 `uninstall_openclaw_twinbox.sh`。
