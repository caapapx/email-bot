...
twinbox                      # task-facing CLI 入口
  ...
  host                       # 宿主集成（bridge 等）
    bridge
      install | remove | status | poll

  openclaw                   # OpenClaw 原生 onboarding 工具（JSON）
    onboarding-start
    onboarding-status
    onboarding-advance
    onboarding-confirm-push SESSION_TARGET
  ...
  daemon                     # 后台 JSON-RPC（Unix socket；省 Python 冷启动）
    start [--supervise] | stop | restart [--supervise] | status [--json]   # --supervise 可选；默认不加
...
twinbox onboard openclaw [--repo-root PATH] [--openclaw-home PATH] [--dry-run]
                         [--openclaw-bin NAME] [--skip-bridge] [--no-start-daemon] [--json]
```

**说明**：

- 这是默认推荐入口，面向人工操作员。
- **Apply setup** 成功后，`deploy openclaw` 会执行与 `twinbox deploy openclaw` 相同的 **Phase 2 前置包**：校验 `openclaw.json` 中 Twinbox skill/plugin 可观测项、安装 **vendor-safe** `twinbox host bridge` user timer、做一次 bridge health（dry-run poll）。JSON 报告含 `phase2_ready`；仅当其为 `true`（或你显式传 `--skip-bridge`）时向导才视为可交接 Phase 2 对话 onboarding。
...
- 部署成功后，默认会尝试 **`twinbox daemon start`**（无额外输出干扰 journey；结果记在 deploy 步骤 `daemon_start`，状态为 `ok` / `skipped` / `failed`）。已运行中的 daemon 会记为 `skipped`。传 **`--no-start-daemon`** 可跳过。
...
twinbox deploy openclaw [--repo-root PATH] [--openclaw-home PATH] [--dry-run] [--no-restart]
                        ...
                        [--openclaw-bin NAME] [--skip-bridge] [--twinbox-bin PATH]
                        [--no-start-daemon] [--json]
twinbox deploy openclaw --rollback [--remove-config] [--dry-run] [--no-restart] [--json]
```

**说明**：

- 成功部署后 JSON 报告含 `phase2_ready`、`plugin_tools`、`bridge` 等字段（与 `onboard openclaw --json` 同源）；步骤列表中可能多一项 **`daemon_start`**（部署成功后尝试启动 JSON-RPC daemon，除非 **`--no-start-daemon`**）。
- `--skip-bridge`：高级逃生口，跳过 bridge 安装/健康检查（不推荐默认使用）。
- `--twinbox-bin`：写入 systemd unit 时使用的 **绝对路径** twinbox 可执行文件（默认自动解析）。
- `--rollback`：撤销上述接线（删除 `skills.entries.twinbox`、`~/.openclaw/skills/twinbox/`），并 **移除** vendor-safe bridge user units（`twinbox host bridge remove` 等价物）；**不删除** `~/.twinbox`；全量卸载见 `integrations/openclaw/DEPLOY.md` §5 `uninstall_openclaw_twinbox.sh`。
...
- `--fragment` / `--no-fragment`：可选将 JSON 片段深度合并进 `openclaw.json`（在写入 `skills.entries.twinbox` 之前）；默认若存在 `integrations/openclaw/openclaw.fragment.json` 则读取。示例见 `integrations/openclaw/openclaw.fragment.example.json`。
...
```bash
twinbox host bridge install [--dry-run] [--no-start] [--openclaw-bin NAME] [--twinbox-bin PATH] [--repo-root PATH]
twinbox host bridge remove [--dry-run] [--repo-root PATH]
twinbox host bridge status [--openclaw-bin NAME] [--twinbox-bin PATH] [--repo-root PATH]
twinbox host bridge poll [--dry-run] [--limit N] [--openclaw-bin NAME] [--format json|text] [--repo-root PATH]
```

### openclaw（插件对齐的 JSON 子命令）

供 OpenClaw 插件 `twinbox_onboarding_*` 调用；输出恒为 JSON。

```bash
twinbox openclaw onboarding-start
twinbox openclaw onboarding-status
twinbox openclaw onboarding-advance [--profile-notes …] [--calibration-notes …] [--cc-downweight on|off]
twinbox openclaw onboarding-confirm-push SESSION_TARGET [--daily on|off] [--weekly on|off] [--openclaw-bin NAME]
```
...
```bash
twinbox push subscribe SESSION_ID [--min-urgency high|medium|low] [--daily on|off] [--weekly on|off] [--json]
```

### push configure

调整已有订阅的 **daily / weekly** cadence，并按订阅聚合结果 enable/disable `daily-refresh` 与 `weekly-refresh` OpenClaw cron 同步。
...
- **Python daemon**：`twinbox daemon start|stop|restart|status [--json]`。**默认不带** `--supervise`（`onboard` / `deploy` 后的自动 `daemon start`、Go 懒拉起同理）；仅当你显式需要「daemon 异常退出后由监督进程再起」时才加 `daemon start --supervise` 或 `restart --supervise`。`status --json` 在曾以 supervise 启动时会返回 `supervised` / `supervisor_pid`。协议与路径见 [daemon-and-runtime-slice.md](./daemon-and-runtime-slice.md)。
