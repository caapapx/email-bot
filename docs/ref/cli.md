...
- **Python daemon**：`twinbox daemon start|stop|restart|status [--json]`（`onboard` / `deploy` 后的自动 `daemon start`、Go 懒拉起同理）。协议与路径见 [daemon-and-runtime-slice.md](./daemon-and-runtime-slice.md)。
